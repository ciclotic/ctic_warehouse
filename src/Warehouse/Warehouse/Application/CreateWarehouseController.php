...
use CTIC\Warehouse\Warehouse\Infrastructure\Controller\WarehouseController;
use CTIC\Warehouse\Warehouse\Infrastructure\Form\Type\WarehouseType;
...
class CreateWarehouseController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new WarehouseType(Warehouse::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Warehouse\Warehouse\Infrastructure\Form\Type\WarehouseType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateWarehouse();
        }
        if(empty($command->resource))
        {
            $command->resource = Warehouse::class;
        }
        if(empty($command->command))
        {
            $command->command = new WarehouseCommand();
        }
    }
